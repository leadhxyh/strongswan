...
public class CharonVpnService extends VpnService implements Runnable
{
	...
	{
		startForeground(VPN_STATE_NOTIFICATION_ID, buildNotification(State.CONNECTING));
	}

	/**
	 * Update the currently shown notification.
	 */
	private void updateNotification(State state)
	{
		synchronized (this) {
			if (mCurrentProfile != null) {
				...
				manager.notify(VPN_STATE_NOTIFICATION_ID, buildNotification(state));
			}
		}
	}

	/**
	 * Build a notification matching the given state
	 *
	...
	private Notification buildNotification(State state)
	{
		android.support.v4.app.NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
				.setContentText(mCurrentProfile.getName())
	...
	public void updateStatus(int status)
	{
		switch (status)
		{
			...
			case STATE_CHILD_SA_UP:
				...
				updateNotification(State.CONNECTED);
				break;
